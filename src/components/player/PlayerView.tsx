...
function CipherGame({ answer, onSolve }: { answer: string; onSolve: () => void }) {
  ...
  const [attempts, setAttempts] = useState(0);
  // Fixed at 3 and announced, this was a lookup, not a puzzle. Random shift,
  // and the amount is earned rather than given.
  const [shift] = useState(() => 2 + Math.floor(Math.random() * 11)); // 2–12
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-[10px] text-text-muted mb-4">One word — the missing word from the clue, not the place name</p>
      {attempts >= 1 && attempts < 2 && (
        <p className="text-xs text-accent mb-2">Hint: it starts with &quot;{cleanAnswer[0]}&quot;</p>
      )}
      {attempts >= 2 && (
        <p className="text-xs text-accent mb-2">
          Hint: it starts with &quot;{cleanAnswer[0]}&quot;, and every letter was shifted {shift} forward
        </p>
      )}
      ...
      {attempts >= 3 && (
        <div className="animate-fade-in bg-surface/60 border border-border/60 rounded-xl p-4 mt-3">
          <p className="text-xs text-text-dim mb-1">The missing word is:</p>
          <p className="text-lg font-bold text-accent mb-3">{cleanAnswer}</p>
          <button onClick={onSolve} className="btn-primary">Got it — continue →</button>
        </div>
      )}
              ...
              <div className="animate-fade-in">
                      ...
                      {!showGiveUp ? (
                        ...
                        <div className="animate-fade-in bg-surface/60 border border-border/60 rounded-xl p-4 mt-2">
                          {activeCp.clue_type !== 'text' && !clueSolved ? (
                            ...
                            <>
                              <p className="text-xs text-text-dim mb-3">Head to the next stop and we&apos;ll point you at it on the map.</p>
                              <button onClick={() => { setShowGiveUp(false); setLocationRevealed(true); setPhase('travel'); }} className="btn-primary">Show me on the map →</button>
                            </>
                          )}
                        </div>
                      )}
                ...
                {phase === 'verify' && (
                  <div className="card animate-fade-in">
                    ...
                    <div className="mt-3 text-center">
                      {!showGiveUp ? <button onClick={() => setShowGiveUp(true)} className="w-full py-3 text-sm text-text-muted hover:text-text-dim cursor-pointer bg-transparent border-none">Stuck? →</button>
                      : <div className="animate-fade-in bg-surface/60 border border-border/60 rounded-xl p-4 mt-2">
                          <p className="text-xs text-text-dim mb-3">No worries. We&apos;ll show you where it is on the map.</p>
                          <button onClick={() => { setShowGiveUp(false); setLocationRevealed(true); setPhase('travel'); }} className="btn-primary">Show me on the map →</button>
                        </div>}
                    </div>
                  </div>
